...
@ActiveProfiles("test")
@Transactional
@Sql("/sql/controller/ReviewControllerTest/base-state.sql")
public class ReviewControllerTest {
	...
	public void 未ログインの場合はレビューを登録せずにログインページにリダイレクトする() throws Exception {
		...
		mockMvc.perform(
				post("/restaurants/1/reviews/create").with(csrf()).param("score", "5").param("content", "テスト感想"))
				.andExpect(status().is3xxRedirection())
				.andExpect(redirectedUrl("http://localhost/login"));
	...
	public void 未ログインの場合はレビューを更新せずにログインページにリダイレクトする() throws Exception {
		mockMvc.perform(
				post("/restaurants/2/reviews/1/update").with(csrf()).param("score", "5").param("content", "テスト感想"))
				.andExpect(status().is3xxRedirection())
				.andExpect(redirectedUrl("http://localhost/login"));
		...
		assertThat(review.getContent())
				.isEqualTo("名古屋では有名な格安で焼肉食べ放題のお店。タイミングよく仕事で行く機会があったので、地元の友人と一緒に来店しました。店内は広くゆったりとできます。");
	}
	...
	public void 有料会員としてログイン済みの場合は他人のレビューを更新せずに店舗詳細ページにリダイレクトする() throws Exception {
		mockMvc.perform(
				post("/restaurants/2/reviews/2/update").with(csrf()).param("score", "5").param("content", "テスト感想"))
				.andExpect(status().is3xxRedirection())
				.andExpect(redirectedUrl("/restaurants/2"));
